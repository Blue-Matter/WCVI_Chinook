...
(ref:fig-fry-surv2) Top: The environmental variable sampled for the projection of Scenario C by bootstrapping historical values. Bottom: the corresponding egg-fry survival by converting the environmental variable using the regression in Figure \@ref(fig:env), with stochasticity using the residual standard deviation.
...
B. Same as A with freshwater survival centred around above average historical autumn freshwater conditions

C. Same as A with freshwater survival sampled stochastically from bootstrapping historical autumn freshwater conditions

D. Same as A with 25% lower ocean exploitation rates from both the preterminal and terminal fisheries

E. Same as A with declining maturity and fecundity. The Sarita ERM estimates declining maturity (earlier age of maturity) in recent decades. 

Scenarios A-D use a recent mean, but scenario E uses the trend during 1990--2018 and continues the trend into the projection (Figure \@ref(fig:mat-trend)). Fecundity at age decreased during the projection (Figure \@ref(fig:fec-trend)). A 5 percent reduction in size at age per decade was used, and the corresponding fecundity was calculated from the fecundity-at-length model in @brown2025.
